...
Tiny URL:
    ...
    tiny URL, find out the requirements for the short URL. Few things that we should ask
    them is:
1. MVP  Minimal Viable Project.
    ...
    Requirements.
2. Calculations:
    We do some random calculations to make an estimate about size
    of database we need and how many servers we need.
    a. The interview says 1 billion users, (1 Billion = 10
    10) are to be served. If we
    have 1 billion users are coming on daily basis, then we would have
    1,157,40 users / second. As per the read to write ratio (1:10), we would
    have 11,574 would be writer and 104,166 would be readers. If
    interviewer says on an average, one computer can handle 200 users, then
    we need approximately 6 computers for master and the same number for
...
3. High Level Picture: of the
architecture.
                The client in his browser hit
                Tiny URL. In the box, he will
                have two options, one is a
                Tiny URL and the other is
                new URL. The Tiny URL will
                check the mapping if the Tiny
                URL is already mapped to a
                particular website previously.
                This is the high-level view of
                the system.
